...
const activateUser = async () => {
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      formData.append('mobile', editedInfo.phone);
      if (editedInfo.image) {
        const reader = new FileReader();
        reader.readAsDataURL(editedInfo.image);
        reader.onload = () => {
  ...
  const updateUserDetails = async (formData) => {
    try {
      const token = localStorage.getItem('token');
      const response = await Axios.patch(`https://gym-backend-apis.onrender.com/admin/user/${user.ID}`, {
        ...formData, 
      }, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      {user ? (
        <div className="bg-white p-4 rounded shadow-md w-full">
          ...
          <div className="flex justify-end">
            <img src={user.IMAGE} alt="User" className="max-w-full h-auto rounded" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 w-full">
            ...
            <div>
                  <label className="text-xl font-semibold mb-2">Next Due:</label>
                  <input
                    type="text"
                    value={user.nextDue}
                    className="w-full p-2 border rounded"
    ...
    Download Payments
          ...
          {isEditing && (
            ...
            <label htmlFor="dob">Date of Birth:</label>
              <DatePicker
                selected={editedInfo.dob ? new Date(editedInfo.dob) : null}
                onChange={(date) => setEditedInfo({ ...editedInfo, dob: date.toISOString().split('T')[0] })}
